...
public class WildCardMatching {
    public boolean isMatch(String s, String p) {
        ...
        for ( int i = 0 ; i < pattern.length; i++) {
            if (pattern[i] == '*') {
                if (isFirst) {
                    //writeIndex++ means it access the value with its original value before incrementing
                    // pattern[writeIndex++] is equivalent to pattern[writeIndex], then writeIndex++
                    pattern[writeIndex++] = pattern[i];
                    isFirst = false;
                }
            } else {
                System.out.println(pattern[writeIndex] + " b: " + writeIndex);

                pattern[writeIndex] = pattern[i];
                writeIndex++;
                //System.out.println( " a: " + writeIndex);
                //System.out.println(wcm.isMatch("abcdefg", "a*e?g"));
                isFirst = true;
            }
        }
